...
G00, G01, G02, G03, G04, G05, G06, and G07 are complete; G08 is ready. Every later goal remains queued behind its listed dependencies. A suitable Luna instruction for the next cycle is:

```text
Implement docs/goals/G08-signal-engine.md. Read the Riff PRD and docs/goals/README.md first, remain within G08 scope, follow its execution contract, verify every acceptance criterion, and return the completion report defined by the roadmap.
```
