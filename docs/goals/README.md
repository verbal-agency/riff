...
G00, G01, G02, G03, G04, and G05 are complete; G06 is ready. Every later goal remains queued behind its listed dependencies. A suitable Luna instruction for the next cycle is:

```text
Implement docs/goals/G06-capability-model.md. Read the Riff PRD and docs/goals/README.md first, remain within G06 scope, follow its execution contract, verify every acceptance criterion, and return the completion report defined by the roadmap.
```
